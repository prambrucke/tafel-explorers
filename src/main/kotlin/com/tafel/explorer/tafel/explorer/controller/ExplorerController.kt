...
class ExplorerController(var explorerService: ExplorerService, val objectMapper: ObjectMapper) {
    ...
    fun createExplorer(@RequestBody explorer: ExtendedExplorer): ResponseEntity.HeadersBuilder<*>{
        return ResponseEntity.created(URI.create("/explorers/"+explorerService.createExplorer(explorer)))
    }

    @GetMapping("explorers/{explorer-id}")
    fun getExplorerById(@PathVariable("explorer-id") explorerId: String): ResponseEntity<ExtendedExplorer>{
        return ResponseEntity.ok(explorerService.getExplorerById(explorerId))
    }

    @PutMapping("explorers/{explorer-id}")
    fun updateExplorerById(@PathVariable("explorer-id") explorerId: String, @RequestBody explorer: Explorer): ResponseEntity.HeadersBuilder<*> {
